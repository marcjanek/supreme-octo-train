...
public class DriverTestsFourXJunction {
	...
	private final Driver driver = new Driver(database, randomGenerator);
	...
	void shouldOneCarTurnLeftAndGoneAndOtherRunStraightToX2() {
		...
		List<Integer> generatorSequence = Arrays.asList(0, 1, 2);//left, right, straight
		...
		performTurn(coordinatesToCarNumbers, generatorSequence, greenLights);
	}

	@Test
	@Order(3)
	void shouldOneCarTurnToX4OneRunStraightToX1AndOneTurnRightAndGone() {
		List<Integer> generatorSequence = Arrays.asList(2, 0);// straight, left
		final Map<Coordinate, Long> coordinatesToCarNumbers = new HashMap<>();
	...
	void shouldOneCarTurnLeftAndGoneAndOtherRunStraightAndGone() {
		List<Integer> generatorSequence = Collections.emptyList();
		final Map<Coordinate, Long> coordinatesToCarNumbers = new HashMap<>();
	...
	void performTurn(Map<Coordinate, Long> coordinateToCarNumbers, List<Integer> randomGeneratorValues, List<Coordinate> greenLights) {
		randomGenerator.addValues(randomGeneratorValues);
